...
def gen_obstacle_map(moving_obstacle=None):
    # identical to your original, but now optionally tests moving_obstacle.contains()
    map_img = np.zeros((HEIGHT, WIDTH, 3), dtype=np.uint8)

    static_obs = [
        lambda x, y: (100 <= x <= 110) and (0 <= y <= 200),
        lambda x, y: (210 <= x <= 220) and (100 <= y <= 300),
    ...
    for y in range(HEIGHT):
        for x in range(WIDTH):
            if any(obs(x, y) for obs in static_obs) \
               or (moving_obstacle and moving_obstacle.contains(x, y)):
                map_img[y, x] = (0, 0, 255)
...
class Node:
    def __init__(self, x, y):
        ...
        self.parent = None
class RRT:
    def __init__(self, start, goal, obstacle_map, visualize_map,max_iter, step_size, goal_sample_rate, goal_threshold,robot_radius, clearance,moving_obstacle=None):
    ...
    # 2) regenerate raw + expanded maps
            raw     = gen_obstacle_map(self.moving_obstacle)
            exp1, mask1 = expand_obstacles(raw, self.robot_radius)
            exp2, mask2 = expand_obstacles(exp1, self.clearance)


    # 3) convert to binary occupancy
            self.obstacle_map = np.where(mask2 == 255, 1, 0).astype(np.uint8)

    # 4) (optional) update self.visualize_map so the moving obstacle shows up
            self.visualize_map = exp2.copy()
            self.visualize_map[np.where(raw[:,:,2] == 255)] = (255,0,0)  
...
def main():
    try:
        ...
        expanded_map, expanded_mask  = expand_obstacles(raw_map, int(np.ceil(RRADIUS*100)))
        ...
        rrt = RRT(start, goal,obstacle_map ,expanded_map_2,max_iterations,step_size,goal_sample_rate,goal_threshold,robot_radius,clearance,moving_obstacle)
